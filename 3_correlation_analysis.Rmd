...
```{r}
#save the data
write.csv(data, "data/train_var_selection.csv", row.names = FALSE)
```

```{r}
#Do the same for the test data
test <- read.csv("data/test_raw.csv", header = TRUE)
test <- test[, !(names(test) %in% columns_to_remove)]
#Save the test data
write.csv(test, "data/test_var_selection.csv", row.names = FALSE)
```